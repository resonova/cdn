...
/* exported
    ...
    FileOutput
    InactiveFileOutput
    ContainerVisibilityOutput
    HideAlternativeOutput
    CompoundOutput
    Transformer
    NoOpTransformer
    MakeUrlSafeTransformer
    TakeUrlSafeTransformer
    SimplifiedBase64Encoder
    SimplifiedBase64Decoder
    FullBase64Encoder
    FullBase64Decoder
    UrlEncoder
    UrlDecoder
    HtmlEncoder
    HtmlDecoder
    AllNonAlphanumericHtmlEncoder
*/

class Action {
    constructor(input, transformer, output) {
        this.input = input;
    ...
    act() {
        try {
            this.input.read((value) => {
                const transformed = this.transformer.transform(value);
                this.output.show(value, transformed);
            }, this.errorHandler);
        } catch (ex) {
            console.error(ex);
            alert(ex);
    ...
    errorHandler(evt) {
        console.error('Failed with error: ', evt);
        alert(`Failed with error: '${evt.target.error.name}'`);
    }
}
...
class OneOrOtherInput extends Input {
    constructor(first, second, firstPredicate) {
        super();
        this.first = first;
        this.second = second;
    ...
    read(onsuccess, onfailure) {
        if (this.firstPredicate()) {
            this.first.read(onsuccess, onfailure);
        } else {
            this.second.read(onsuccess, onfailure);
        }
    }
}
...
class HtmlInput extends Input {
    ...
    read(onsuccess, onfailure) {
        try {
            const input = $(this.inputElementId).val();
            onsuccess(input);
        } catch (ex) {
            onfailure(ex);
        }
...
class FileInput extends Input {
    ...
    read(onsuccess, onfailure) {
        ...
        this.readFile(reader, this.fileInput.files[0]);
    }
}

class TextFileInput extends FileInput {
    readFile(reader, file) {
        reader.readAsText(file);
    }
}

class BinaryFileInput extends FileInput {
    readFile(reader, file) {
...
class HtmlOutput extends Output {
    constructor(outputId, outputPanelId) {
        super();
        this.outputId = outputId;
        this.outputPanelId = outputPanelId;
    }

    show(input, result) {
        try {
            const outputPanel = $(this.outputPanelId);
            $(this.outputId).text(result);
            if (!outputPanel.is(':visible')) {
                outputPanel.show(0);
            }
        } catch (ex) {
            console.error(ex);
    ...
    hide() {
        $(this.outputId).text('');
        $(this.outputPanelId).hide(0);
    }
}

class InactiveHtmlOutput extends Output {
    constructor(outputId, outputPanelId) {
        super();
        this.outputId = outputId;
        this.outputPanelId = outputPanelId;
...
class FileOutput extends Output {
    constructor(containerId, filenameId, outputLinkId, encoder = new FullBase64Encoder()) {
        super();
        ...
        const outputLink = $(this.outputLinkId);
        const filenameField = $(this.filenameId);
        filenameField.on('keyup', () => {
            outputLink.attr('download', filenameField.val());
        });
    }

    show(input, result) {
        $(this.containerId).show(0);
        const resultToDisplay = this.encoder.transform(result);
        const filename = $(this.filenameId).val();
        const link = $(this.outputLinkId);
        link.attr('download', filename);
        link.attr('href', `data:application/octet-stream;charset=utf-16le;base64,${resultToDisplay}`);
    }
...
class InactiveFileOutput extends Output {
    constructor(containerId, filenameId, outputLinkId, encoder = new FullBase64Encoder()) {
        super();
        this.containerId = containerId;
        this.filenameId = filenameId;
...
class ContainerVisibilityOutput extends Output {
...
class HideAlternativeOutput extends Output {
    constructor(outputToShow, outputToHide) {
        super();
        this.outputToShow = outputToShow;
        this.outputToHide = outputToHide;
    }

    show(input, result) {
        this.outputToHide.hide();
        this.outputToShow.show(input, result);
    }

    hide() {
        this.outputToShow.hide();
    }
}

class CompoundOutput extends Output {
    constructor() {
        super();
        this.outputs = Array.prototype.slice.call(arguments);
    }

    show(input, result) {
        this.outputs.map((output) => {
            // output.hide();
            output.show(input, result);
        });
    }
...
class Transformer {
    constructor(previous = new NoOpTransformer()) {
...
class MakeUrlSafeTransformer extends Transformer {
    constructor(previous) {
        super(previous);
    }

    transform(input) {
        const previous = this.previous.transform(input);
        const result = previous.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+/g, '');

        return result;
    }
}

class TakeUrlSafeTransformer extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const result = this.convertFromUrlSafeIfRequired(previous);

        return result;
    }

    convertFromUrlSafeIfRequired(toDecode) {
        let padded = '';
        const paddingOffset = toDecode.length % 4;
        switch (paddingOffset) {
            case 0:
                padded = toDecode;
                break;
            case 1:
...
class SimplifiedBase64Encoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const result = btoa(previous);

        return result;
    }
}

class SimplifiedBase64Decoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const result = atob(previous);

        return result;
    }
}

// Instead of the regular atob(), use Mozilla's StringView for better Unicode handling.
class FullBase64Encoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const stringView = new StringView(previous, 'BinaryString');
        const result = stringView.toBase64();

        return result;
    }
}

// Instead of the regular btoa(), use Mozilla's StringView for better Unicode handling.
class FullBase64Decoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const result = StringView.makeFromBase64(previous);

        return result;
    }
}

class UrlEncoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const result = encodeURIComponent(previous);

        return result;
    }
}

class UrlDecoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const result = decodeURIComponent(previous);

        return result;
    }
}

class HtmlEncoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        ...
        const result = temporaryDiv.text(previous).html();
        temporaryDiv.remove();
...
class HtmlDecoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
    ...
    transform(input) {
        const previous = this.previous.transform(input);
        const temporaryDiv = $('<div/>');
        const result = temporaryDiv.html(previous).text();
        temporaryDiv.remove();
...
class AllNonAlphanumericHtmlEncoder extends Transformer {
    constructor(previous) {
        super(previous);
    }
